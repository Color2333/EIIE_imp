...
def _load_from_summary(index, config):
    ...
    history_series = dataframe.loc[int(index)]["backtest_test_history"]
    config_series = dataframe.loc[int(index)]["config"]

    # 处理pandas Series对象，获取实际的字符串值
    if hasattr(history_series, 'iloc'):
        history_string = history_series.iloc[0] if len(history_series) > 0 else history_series
    else:
        history_string = history_series

    if hasattr(config_series, 'iloc'):
        config_value = config_series.iloc[0] if len(config_series) > 0 else config_series
    else:
        config_value = config_series

    if not check_input_same(config, json.loads(str(config_value))):
        raise ValueError("the date of this index is not the same as the default config")

    return np.fromstring(str(history_string), sep=",")[:-1]
